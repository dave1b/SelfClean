...
from selfclean.core.src.trainers.text.mlm_trainer import MlmTrainer
...
MLM_TEXT_STANDARD_HYPERPARAMETERS = {
    "optimizer": {
        "name": "adamw",
        "args": {}
    },
...
def train_mlm_text(
    ...
    hyperparameters: dict = MLM_TEXT_STANDARD_HYPERPARAMETERS,
    ...
    assert all(
        key in hyperparameters for key in MLM_TEXT_STANDARD_HYPERPARAMETERS
...
if __name__ == "__main__":
    start = datetime.now()
    tokenizer = get_encoder_tokenizer_class(MLM_TEXT_STANDARD_HYPERPARAMETERS['model']['base_model'])[1]
    ...
    logger.info("Training MLM Text")
    model = train_mlm_text(train_dataset, val_dataset, 35, 16, True, 1, None, MLM_TEXT_STANDARD_HYPERPARAMETERS)
    logger.info(f'Finished MLM training after: {datetime.now() - start}')

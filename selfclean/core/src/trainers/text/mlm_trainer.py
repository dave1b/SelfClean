...
from ...models.text.mlm.model import MlmModel
...
class MlmTrainer(Trainer):
    def __init__(
        ...
        super().__init__(
            ...
            arch_name=f"MLM_{train_dataset.dataset.name}",
        ...
        self.model = MlmModel(self.config["model"]["base_model"], self.config["model"]["mlm_probability"])
    ...
    def _train_epoch(self, epoch: int, optimizer, lr_schedule, wd_schedule, n_iter: int) -> float:
        ...
        for batch in self.train_dataset:
            ...
            with autocast(dtype=torch.bfloat16):
                logits, rand_mask = self.model(sentences["input_ids"], sentences["attention_mask"])
            ...
            if n_iter % 25 == 0:
                with torch.no_grad():
                    entropy = calculate_embedding_entropy(logits.cpu())
